...
# 
# Purpose: Table 4 (column 1) in the manuscript
# Note, Table D2 (column 1) in Appendix D uses almost identical code except with RP-CB data
# 
# Input: 
#       1. r-output/2017-sp-estimates: estimates with 2018 survey CB data
# Output:
#       1. results/original/sp/2017-welfare-all.csv: full simulation results
###################################################################
...
#Import data
#-----------------------------------------------------------------#
load("r-output/2017-sp-estimates") 
...
wtpall <- as.data.frame(do.call(rbind, welfare)) %>%
  mutate(sim_id=rep(1:30, 1285)) %>%
  mutate(id=rep(1:1285, each=30))

#############################################################################################
#Export
#############################################################################################
write.csv(wtpall, "results/original/sp/2017-welfare-all-09-08.csv", row.names=FALSE)
