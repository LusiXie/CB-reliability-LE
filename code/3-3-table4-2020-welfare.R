###################################################################
# Author: Lusi Xie, University of Alberta, 01/24/2022

# Purpose: Table 4 (column 4) in the manuscript
# Note, Table D2 (column 4) in Appendix D uses almost identical code except with RP-CB data
# 
# Input: 
#       1. r-output/2019-sp-estimates: estimates with 2020 survey CB data
# Output:
#       1. results/original/sp/2019-welfare-all.csv: full simulation results
###################################################################
...
#Import data
#-----------------------------------------------------------------#
load("r-output/2019-sp-estimates")
...
wtpall <- as.data.frame(do.call(rbind, welfare)) %>%
  mutate(sim_id=rep(1:30, 2236)) %>%
  mutate(id=rep(1:2236, each=30))

#############################################################################################
#Export
#############################################################################################
write.csv(wtpall, "results/original/sp/2019-welfare-all-09-08.csv", row.names=FALSE)
